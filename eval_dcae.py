...
from src.lightning_dcae_eval_model import LightningDCAEEvalModel
...
class ReWriteRootSaveConfigCallback(SaveConfigCallback):
    def save_config(
        ...
        file_path = os.path.join(
            trainer.default_root_dir, f"config-{stage}-{stamp}.yaml"
        )
        self.parser.save(
            self.config,
            file_path,
...
class ReWriteRootDirCli(LightningCLI):
    ...
    def instantiate_trainer(self, **kwargs: Any) -> Trainer:
        ...
        if os.path.exists(default_root_dir) and "debug" not in default_root_dir:
            if (
                os.listdir(default_root_dir)
                and self.subcommand != "predict"
                and not is_resume
            ):
