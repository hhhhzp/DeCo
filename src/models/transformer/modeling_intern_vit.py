...
class InternVisionEmbeddings(nn.Module):
    def __init__(self, config: InternVisionConfig):
        ...
        # Initialize learnable tokens if num_learnable_tokens > 0
        if self.num_learnable_tokens > 0:
            self.query_tokens = nn.Parameter(
                torch.randn(1, self.num_learnable_tokens, self.embed_dim),
                requires_grad=True,
            )
            # Xavier uniform initialization (recommended for Transformer)
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        # Append learnable tokens at the end if they exist
        if self.query_tokens is not None:
            learnable_tokens_expanded = self.query_tokens.expand(batch_size, -1, -1).to(
                target_dtype
            )
            embeddings = torch.cat([embeddings, learnable_tokens_expanded], dim=1)
