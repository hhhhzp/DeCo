...
class UniFlowVisionModel_DCAE(PreTrainedModel):
        ...
        if self.enable_pixel_branch:
            if self.use_chal_proj:
                self.gen_proj = nn.Sequential(
                    ...
                    nn.GELU(),
                    nn.Linear(4 * vit_hidden_size, 32),
                )
    ...
    def forward_pixel_decoder(self, latent_tokens, target_pixels=None, training=True):
        ...
        Args:
            latent_tokens: latent tokens [B, N, latent_ch=32]
        ...
        if training:
            ...
            reconstructed_image = self.flow_head(z=condition_tokens, pos=None)
