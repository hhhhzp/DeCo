...
class UniFlowVisionModel(PreTrainedModel):
    ...
    # ============================================================
    def encode_latent(self, gen_tokens, sem_tokens):
        """
        Encode tokens to latent representations.

        Args:
            gen_tokens: generation tokens [B, N, C]
            sem_tokens: semantic tokens [B, N/4, 4*vit_hidden_size]

        Returns:
            latent_tokens: latent tokens for generation [B, N, latent_ch]
            sem_latent_tokens: latent tokens for semantic reconstruction [B, N/4, 128]
        """
        # Project generation tokens to latent space
    ...
    def forward_loss(self, target_pixel_values, teacher_feat=None):
        ...
        Returns:
            dict: loss components including:
                - loss: total loss
                - flow_loss: pixel reconstruction MSE loss
                - lpips_loss: perceptual loss
                - distill_loss: distillation loss
                - sem_mse_loss: semantic reconstruction loss
        """
        # Step 1: Forward encoder
    ...
    def forward(self, pixel_values):
        ...
        # Step 2: Encode latent
        latent_tokens, sem_latent_tokens = self.encode_latent(gen_tokens, sem_tokens)
