...
class UniFlowVisionModel(PreTrainedModel):
    ...
    def forward(
        ...
        Args:
            ...
            normalize_type: normalization type for input images
            return_distill_loss: if True, return distill_loss along with semantic tokens (only for semantic mode)

        Returns:
            If mode='pixel':
                reconstructed_image: [B, C, H, W]
            If mode='semantic' and return_distill_loss=False:
                sem_tokens_pred_after_mlp: [B, N/4, llm_hidden_size]
            If mode='semantic' and return_distill_loss=True:
                (sem_tokens_pred_after_mlp, distill_loss): tuple of semantic tokens and distillation loss
        """
        # Validate mode
